...
export default function QueryEditor() {
  ...
  return (
    <div className="bg-bg-card rounded-[10px] border border-border-primary overflow-hidden shrink-0 shadow-sm relative flex items-start">
      <div className="flex items-center gap-1.5 py-3 pl-3.5 pr-0 shrink-0">
        <span className="font-mono text-[12px] font-semibold text-accent bg-accent-bg px-2 py-0.5 rounded">
          {dbType === "neo4j" ? selectedDb : dbType}
        </span>
        <span className="font-mono font-bold text-[14px] text-accent select-none">$</span>
      </div>
      <textarea
        className="w-full py-2.5 pr-[60px] pl-2 border-none outline-none resize-none font-mono text-[13px] text-text-code bg-transparent leading-[1.6] h-[72px] placeholder:text-text-faint"
        value={query}
